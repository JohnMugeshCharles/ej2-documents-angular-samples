...
const sbArray: string[] = ['nextjs', 'react', 'ts', 'javascript', 'asp_core', 'asp_mvc', 'vue', 'blazor'];
...
export class SBController {
    ...
    constructor(
        ...
        this.sourceTab = new Tab({
            ...
            headerPlacement: 'Bottom', cssClass: 'sb-source-code-section', created: this.dynamicTabCreation, selected: function (e: any) {
                ...
                blockEle.innerHTML = this.items[e.selectedIndex].data.replace(reg, '');
    ...
    breadCrumbUpdate(controlName: string, category: string, sampleName: string) {
        ...
        else if (controlName === 'PDF Viewer') {
            canonicalUrl += 'pdf-viewer-sdk/angular-pdf-viewer';
        }
        else if (controlName === 'Word Processor') {
            canonicalUrl += 'docx-editor-sdk/angular-docx-editor';
        }
        else if (controlName === 'Spreedsheet') {
            canonicalUrl += 'spreadsheet-editor-sdk/angular-spreadsheet-editor';
        }
    ...
    dynamicTabCreation(obj: any): void {
        ...
        blockEle.innerHTML = tabObj.items[tabObj.selectedItem].data.replace(reg, '');
        blockEle.classList.add('sb-src-code');
        if (blockEle) {
            hljs.highlightBlock(blockEle);
        }
    ...
    ngOnInit(): void {
        ...
        this.router.events
            ...
            .subscribe((event: any) => {
                ...
                if (href && href.endsWith(suffix)) {  // Check if it ends with 'index.html' and remove it
    ...
    private setSbLink(): void {
        ...
        let ej2PlatformHash: string = `#/${selectedThemes}/${hrefLink.join('/')}`;
        const sdkPlatforms: { [key: string]: string } = {
            nextjs: 'nextjs', javascript: 'javascript-es5', react: 'react', vue: 'vue', ts: 'javascript',
            asp_core: 'asp-net-core', asp_mvc: 'asp-net-mvc', blazor: 'blazor-server'
        };
        let baseUrl: string = location.href.split('#')[0];
        if (baseUrl.endsWith('angular/')) {
            baseUrl = baseUrl.replace('angular/', '');
        } else {
        ...
        for (let i: number = 0, len: number = sbArray.length; i < len; i++) {
            ...
            if (component === 'pdf' && ['react', 'ts', 'javascript', 'asp_core', 'vue'].indexOf(sb) === -1) {
                if (ele) {
            ...
            if (sb === 'asp_core') {
                ...
                sampleUrl += `${selectedThemes}/${hrefLink.join('/')}`;
            } else {
                sampleUrl += ej2PlatformHash;
                if (sb === 'ts' || sb === 'vue' || sb === 'javascript') {
                    sampleUrl += '.html';
                }
            }
            if (ele) {
    ...
    ngAfterViewInit(): void {
        ...
        if (isDeviceMode) {
            this.validateCurrentSampleVisibility();
        }
    ...
    private updateThemeClass(theme: string): void {
        ...
        document.body.className = isDark
            ? `e-view ${baseTheme}-dark e-dark-mode`
            : `e-view ${baseTheme}`;
    ...
    private updateStylesheet(theme: string): void {
        ...
        if (linkEl) {
            linkEl.setAttribute('href', `./styles/${theme}.css`);
        }
    }
    ...
    /**
     * Validates if the currently displayed sample should be hidden in device mode.
     * If the sample is not available in the current device mode (hideOnDevice: true),
     * navigates to the default sample of that component.
     */
    private validateCurrentSampleVisibility(): void {
        try {
            ...
            const theme = hashParts[1];
            const componentName = hashParts[2];
            const sampleName = hashParts[3];

            // Check if current sample exists in the filtered sample list for the current device mode
            // Get the filtered data source (which respects hideOnDevice)
            const filteredDataSource = this.leftControl?.getDataSource?.();
            const currentSampleExists = this.sampleExistsInFilteredList(componentName, sampleName, filteredDataSource || samplesList);

            // If current sample doesn't exist in the filtered list (meaning it's hidden in this device mode)
            // Check if in device mode using the same logic as lp.component.ts
            const isDeviceMode = (window as any).browserDetails?.isDevice;
            if (!currentSampleExists && isDeviceMode) {
                // Navigate to the default sample of this component
                const defaultPath = `${theme}/${componentName}/default`;
                this.router.navigateByUrl(defaultPath);
            }
        } catch (error) {
            console.warn('Error validating sample visibility:', error);
        }
    }

    /**
     * Checks if a sample exists in the filtered sample list (respecting hideOnDevice filter)
     */
    private sampleExistsInFilteredList(componentName: string, sampleName: string, samples: any[]): boolean {
        for (let sample of samples) {
            // Check if this is the main component/module
            if (sample.path === componentName && sample.samples) {
                // Search for the sample within this component
                for (let subSample of sample.samples) {
                    if (subSample.path && subSample.path.includes(sampleName)) {
                        return true;
                    }
                }
            }
    ...
    updateSourceCode(path: string): void {
        ...
        const isAISamples: boolean = /ai-(?!assistview\b)[a-z-]+/.test(pathArray[0]) || ((pathArray[0] === 'spreadsheet' || pathArray[0] === 'document-editor') && (/^ai-.*/).test(pathArray[1]));
